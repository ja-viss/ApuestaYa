 ...
 * @param item - Datos del archivo a descargar.
 * @param index - Índice para el retardo de la animación.
 */
export const DownloadCard: React.FC<DownloadCardProps> = ({ item, index }) => {
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      whileHover={{ y: -10 }}
      className="bg-white/5 backdrop-blur-xl border border-white/10 p-6 rounded-3xl group"
    >
      ...
      <h3 className="text-xl font-bold mb-2 group-hover:text-cyan-400 transition-colors">{item.name}</h3>
